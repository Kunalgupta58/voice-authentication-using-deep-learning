...
class MLEngine:
    ...
    def _load_model(self):
        """Separated model loading logic with retry decorator support."""
        model_dir = Path(BASE_DIR) / "pretrained_models" / "spkrec-ecapa-voxceleb"
        ...
        has_local_model = required_files.issubset({path.name for path in model_dir.iterdir()})

        logger.info(f"Preparing SpeechBrain model from {model_dir}...")
        if not has_local_model:
            logger.info("Local model files missing, downloading from Hugging Face.")
            # Manually download without symlinks to prevent Windows Admin privilege crashes
            snapshot_download(
            ...
            logger.info("Using existing local SpeechBrain model files.")

        # Load state-of-the-art ECAPA-TDNN Model from the local hard-copied dir
